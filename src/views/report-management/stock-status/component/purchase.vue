...
import { fetchPurchaseSumByYear, fetchMonthPurchaseDetail } from '@/api/report'
...
export default {
  ...
  methods: {
    getPurchaseSumByYear() {
      ...
      fetchPurchaseSumByYear({ year: this.year }).then(({ data, code, message }) => {
        if (code === 200) {
    ...
    getPurchaseSumDetail() {
      ...
      fetchMonthPurchaseDetail({ yearAndMonth: this.month }).then(({ data, code, message }) => {
        if (code === 200) {
          if (data) {
