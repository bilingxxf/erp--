<template>
  ...
  <div class="page-container in-warehouse-verify-index">
    ...
    <div class="content-container">
      ...
      >
        ...
        <!-- <el-table-column prop="className" label="状态" align="center">
          <template slot-scope="scope">
            <el-tag :type="materialReturnIndexStatus[scope.row.status].tip" size="medium">{{ materialReturnIndexStatus[scope.row.status].name }}</el-tag>
          </template>
        </el-table-column> -->
        <el-table-column label="操作">
          <template slot-scope="scope">
            <el-button v-permission="['50_202_1']" type="primary" size="small" icon="el-icon-edit-outline" @click="openDetail(scope.row)">退库办理</el-button>
          </template>
        </el-table-column>
      </el-table>
    ...
    <!-- 其他模块（例如弹窗等） -->
    <el-dialog
      :title="`${ materialBaseNum[currentInbound.formType] ? materialBaseNum[currentInbound.formType].name : ''}退库办理：${currentInbound.storageListNo}`"
      :visible.sync="detailVisible"
      :fullscreen="true"
    >
      <div slot="title" class="dialog-title">
        <span>{{ `${ materialBaseNum[currentInbound.formType] ? materialBaseNum[currentInbound.formType].name : ''}退库办理：${currentInbound.storageListNo}` }}</span>
      ...
      <GeneralMat v-if="currentInbound.formType === MATERIAL_BASE_TYPE.material.index" :is-verify="true" :detail-id="currentInbound.id" @closeEvent="detailVisible = false" />
      <SteelPlate v-if="currentInbound.formType === MATERIAL_BASE_TYPE.steelPlate.index" :is-verify="true" :detail-id="currentInbound.id" @closeEvent="detailVisible = false" />
      <Steel v-if="currentInbound.formType === MATERIAL_BASE_TYPE.steel.index" :is-verify="true" :detail-id="currentInbound.id" @closeEvent="detailVisible = false" />
      <StripSteel v-if="currentInbound.formType === MATERIAL_BASE_TYPE.stripSteel.index" :is-verify="true" :detail-id="currentInbound.id" @closeEvent="detailVisible = false" />
      <Enclosure v-if="currentInbound.formType === MATERIAL_BASE_TYPE.enclosure.index" :is-verify="true" :detail-id="currentInbound.id" @closeEvent="detailVisible = false" />
    </el-dialog>
  </div>
</template>
...
import GeneralMat from '@/views/component/returnbound/generalMate'
import SteelPlate from '@/views/component/returnbound/steelPlate'
import Steel from '@/views/component/returnbound/steel'
import StripSteel from '@/views/component/returnbound/stripSteel'
import Enclosure from '@/views/component/returnbound/enclosure'
